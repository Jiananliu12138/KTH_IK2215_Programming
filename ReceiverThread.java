    ...
    public void run() {
		...
		break;
	        ...
	        //synchronized (receiverList) {
				...
				System.err.println("\t RCV ACK\t" + (gudppacket.getSeqno()) + "\tNO MATCHING END POINT. DROP!");
                            } 
                        } /* synchronized (senderList) */
			break;
			...
			// Otherwise, ignore the BSN packet
				...
				case FIRST_BSN:
				    ...
				    if ((gudppacket.getSeqno() <= endPoint.getExpectedseqnum()-endPoint.getWindowSize()) 
				        ...
				        endPoint.setDropReceive(true);
                                        endPoint.setExpectedseqnum(gudppacket.getSeqno());
                                        System.err.println("\t RCV BSN\t" + (gudppacket.getSeqno()) + "\tDROP! ALSO RESET END POINT");
				        receivePacket = false;
					continue;
				    } else {
					// New BSN has probably been dropped once already. So, we don't drop it again.
				    }
				    break;
				case RANDOM:
				    if (Math.random() <= endPoint.getChance()) {
				...
				case FIRST_FIN:
				    ...
				    endPoint.setDropReceive(false);
                                    System.err.println("\t RCV BSN\t" + (gudppacket.getSeqno()) + "\tALSO RESET END POINT");
                                    endPoint.setExpectedseqnum(gudppacket.getSeqno()+1);
                                    sendACK(endPoint, gudppacket);
                                } else {
                                    System.err.println("\t RCV BSN\t" + (gudppacket.getSeqno()) + "\tDROP! endPoint.getFinished()");
                                }
				...
				case FIRST_FIN:
				    ...
				    if (gudppacket.getSeqno() == endPoint.getExpectedseqnum()) {
                                        ...
                                        System.err.println("\t RCV BSN\t" + (gudppacket.getSeqno()) + "\tIGNORE! NOT EXPECTED SEQ");
					// also resend ACK for expectedseqnum
				        gudppacket.setSeqno(endPoint.getExpectedseqnum()-1);
				        sendACK(endPoint, gudppacket);
                                    }
                                } else {
                                    System.err.println("\t RCV BSN\t" + (gudppacket.getSeqno()) + "\tDROP! endPoint not finished");
                                }
			    }
                        } /* synchronized (receiverList) */
                        break;
			...
			// Otherwise, ignore the DATA packet
				...
				case FIRST_FIN:
					...
					this.receiverList.notify();
                                        ...
                                        System.err.println("\t RCV DATA\t" + (gudppacket.getSeqno()) + "\tIGNORE! NOT EXPECTED SEQ");
					// also resend ACK for expectedseqnum
				        gudppacket.setSeqno(endPoint.getExpectedseqnum()-1);
				        sendACK(endPoint, gudppacket);
                                    }
			        ...
			        // we ignore a DATA packet arriving from non-existing end point.
                                System.err.println("\t RCV DATA\t" + (gudppacket.getSeqno()) + "\tIGNORE! NO END POINT");
			    }
                        } /* synchronized (receiverList) */
                        break;
				...
				case FIRST_DATA:
				    ...
				    if (gudppacket.getSeqno() == endPoint.getExpectedseqnum()) {
				        ...
				        endPoint.setFinished(true);
                                        ...
                                        System.err.println("\t RCV FIN\t" + (gudppacket.getSeqno()) + "\tIGNORE! NOT EXPECTED SEQ");
					// also resend ACK for expectedseqnum
				        gudppacket.setSeqno(endPoint.getExpectedseqnum()-1);
				        sendACK(endPoint, gudppacket);
                                    }
